...
module DifferHelper
  ...
  def output(col_sep = "\t")
    ...
    # fail if @result_set.empty?
    base_file_name = "result_#{@search_value.to_s.ljust(4, 'x') || 'all'}"
  ...
  def output_csv_base(file_name, col_sep, attr_name)
    options = { external_encoding: OUTPUT_FILE_ENCODING,
                col_sep: col_sep }
    CSV.open("#{BASE_DIR_NAME}#{file_name}", 'w', options) do |csv|
